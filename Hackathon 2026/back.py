...
def green_score(distance_m, duration_s, profile, fastest_duration_s):
    ...
    practicality_weight = min(0.9, max(0.1, distance_km / 20))
    eco_weight = 1 - practicality_weight

    emissions = distance_m * (emissions_map.get(profile, 0.15) / 1000)
    eco_rating = 100 * (2.718 ** (-emissions / 5000))
    practicality = (fastest_duration_s / duration_s) * 100
...
def route():
    ...
    average_score = user.green_score / user.journey_count


    return jsonify({
        "routes": results,
        "best_route": best,
        "user_score": average_score
    })
